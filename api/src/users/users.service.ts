...
export class UsersService {
    ...
    async findAllPartners() {
        ...
        return partners.map(p => {
            const totalEarnings = p.transactions.reduce((sum, t) => sum + t.amount, 0);
            return {
                id: p.id,
                name: p.name,
                expertise: 'Généraliste', // Placeholder
                tier: totalEarnings > 100000 ? 'platinum' : totalEarnings > 50000 ? 'gold' : 'silver',
    ...
    async changePassword(id: string, dto: ChangePasswordDto) {
        ...
        const isValid = await this.passwordService.comparePassword(dto.oldPassword, user.password);
