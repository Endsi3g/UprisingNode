...
export class DashboardController {
  ...
  async getStats(
    ...
    // Performance Optimization: Run independent queries concurrently using Promise.all
    // Expected impact: Reduces endpoint latency by executing 3 database/service queries in parallel instead of sequentially.
    const [accumulatedGains, user, leads] = await Promise.all([
      this.transactionsService.getTotalEarnings(userId),
      this.prisma.user.findUnique({ where: { id: userId } }), // Get user data for account status
      this.leadsService.findAll(userId), // Get active pipeline
    ]);
  ...
  async getCommissions(@Request() req: AuthenticatedRequest) {
    ...
    // Performance Optimization: Run independent queries concurrently using Promise.all
    // Expected impact: Reduces endpoint latency by executing 4 database/service queries in parallel instead of sequentially.
