...
export class DashboardController {
  ...
  async getStats(
    ...
    // Concurrently fetch all necessary data using Promise.all to avoid waterfall delays
    // and rely on database-level aggregations instead of in-memory processing.
  ...
  async getCommissions(@Request() req: AuthenticatedRequest) {
    ...
    // Concurrently fetch commissions metrics using database aggregations
    // instead of bringing all transactions into memory.
    const [
      totalEarnings,
      pendingEarnings,
      monthlyEarnings,
      ...
      this.transactionsService.findAll(userId), // needed for history
    ]);
