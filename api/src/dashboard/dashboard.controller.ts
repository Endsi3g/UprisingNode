...
export class DashboardController {
  ...
  async getStats(
    ...
    // ⚡ Bolt: Execute independent queries concurrently using Promise.all to prevent database query waterfalls
    // Eliminates sequential awaits for transactions, user, and leads
  ...
  async getCommissions(@Request() req: AuthenticatedRequest) {
    ...
    // ⚡ Bolt: Execute independent queries concurrently using Promise.all to prevent database query waterfalls
    // Eliminates sequential awaits for 4 separate transaction aggregations/queries
