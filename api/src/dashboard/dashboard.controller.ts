...
export class DashboardController {
    ...
    async getStats(@Request() req): Promise<DashboardStats> {
        ...
        const activePipeline = leads
            ...
            .map(l => ({
                ...
                status: (l.status.toLowerCase() as 'analysis' | 'pending' | 'approved') || 'analysis',
    ...
    async getCommissions(@Request() req) {
        const userId = req.user.userId;
        const totalEarnings = await this.transactionsService.getTotalEarnings(userId);
        const pendingEarnings = await this.transactionsService.getPendingEarnings(userId);
        const monthlyEarnings = await this.transactionsService.getMonthlyEarnings(userId);

        // Avg per deal calculation
        const transactions = await this.transactionsService.findAll(userId);
        const paidCommissions = transactions.filter(t => t.type === 'COMMISSION' && t.status === 'PAID');
        const avgPerDeal = paidCommissions.length > 0 ? totalEarnings / paidCommissions.length : 0;
