...
export class DashboardController {
  ...
  async getStats(
    ...
    // Optimized: Run independent queries concurrently via Promise.all
    // Replaced full memory load of leads with DB-level aggregations
  ...
  async getCommissions(@Request() req: AuthenticatedRequest) {
    ...
    // Optimized: Fetch all stats concurrently and replace O(N) filtering with DB counts
    const [
      totalEarnings,
      pendingEarnings,
      monthlyEarnings,
