...
export class DashboardController {
  ...
  async getStats(
    ...
    // Get potential gains directly from db aggregation to avoid O(N) in memory processing
    const potentialGains = await this.leadsService.getPotentialGains(userId);

    // Get active pipeline
    const leadsPipeline = await this.leadsService.getActivePipeline(userId);
    const activePipeline = leadsPipeline.map((l) => ({
      id: l.id,
