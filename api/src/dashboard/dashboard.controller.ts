...
export class DashboardController {
  ...
  async getStats(
    ...
    // ⚡ Bolt: Execute independent DB queries concurrently
    // Expected performance impact: Reduces overall endpoint latency by
    // waiting for the longest query instead of sequential cumulative times
  ...
  async getCommissions(@Request() req: AuthenticatedRequest) {
    ...
    // ⚡ Bolt: Execute independent DB queries concurrently
    // Expected performance impact: Reduces latency by parallelizing 4 database
    // operations instead of sequential waterfall execution
