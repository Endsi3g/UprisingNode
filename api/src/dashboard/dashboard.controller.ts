...
export class DashboardController {
  ...
  async getStats(
    ...
    // ⚡ Bolt Optimization: Use Promise.all to fetch independent database queries concurrently.
    // This prevents a query waterfall where accumulatedGains, user, and leads are fetched sequentially,
    // reducing total response latency by overlapping network and DB I/O.
  ...
  async getCommissions(@Request() req: AuthenticatedRequest) {
    ...
    // ⚡ Bolt Optimization: Batch independent queries concurrently with Promise.all
    // Fetching totalEarnings, pendingEarnings, monthlyEarnings, and transactions in parallel
    // significantly reduces database query waterfalls and response latency.
