...
export class DashboardController {
  ...
  async getStats(
    ...
    // Optimize: Replace in-memory mapping and aggregation with DB methods
    const [potentialGains, rawActivePipeline] = await Promise.all([
      this.leadsService.getPotentialGains(userId),
      this.leadsService.getActivePipeline(userId),
    ]);

    const activePipeline = rawActivePipeline.map((l) => ({
      id: l.id,
