...
export class DashboardController {
  ...
  async getStats(
    ...
    // ⚡ Bolt Optimization: Batch independent DB queries to run concurrently.
    // Expected Impact: Reduces DB I/O wait time by ~60% for this endpoint since 3 sequential calls are parallelized.
  ...
  async getCommissions(@Request() req: AuthenticatedRequest) {
    const userId = req.user.userId;
    // ⚡ Bolt Optimization: Execute independent aggregation queries concurrently.
    // Expected Impact: Reduces latency on the commissions endpoint by avoiding 4 sequential DB round-trips.
