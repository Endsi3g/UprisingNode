...
export class DashboardController {
  ...
  async getStats(
    ...
    // Bolt: Use Promise.all to fetch metrics concurrently and use database aggregations
    // instead of fetching all leads into memory and filtering them O(N).
  ...
  async getCommissions(@Request() req: AuthenticatedRequest) {
    ...
    // Bolt: Use Promise.all to fetch data concurrently. Replace O(N) array filtering
    // with database count for avgPerDeal calculation.
    const [
      totalEarnings,
      pendingEarnings,
      monthlyEarnings,
