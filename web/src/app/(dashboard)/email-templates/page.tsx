...
export default function EmailTemplatesPage() {
  const handleCopy = (text: string, type: "subject" | "body") => {
    ...
    toast.success(
      `${type === "subject" ? "Objet" : "Corps"} copié dans le presse-papier`,
    );
  };
      ...
      <div className="px-6 md:px-12 max-w-7xl mx-auto w-full">
        <Tabs defaultValue="cold" className="w-full">
          <TabsList className="mb-8 bg-gray-100 p-1 rounded-sm w-full md:w-auto flex overflow-x-auto">
            <TabsTrigger
              value="cold"
              className="flex-1 md:flex-none uppercase tracking-widest text-xs data-[state=active]:bg-white data-[state=active]:shadow-sm"
            >
              Cold Outreach
            </TabsTrigger>
            <TabsTrigger
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {TEMPLATES.filter((t) => t.category === category).map(
                  (template) => (
                    <div
                      key={template.id}
                      className="border border-black/10 group bg-white hover:border-black/30 transition-all flex flex-col"
                    >
                      ...
                      <div className="p-6 flex-1 space-y-4">
                        <div>
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <button
                              onClick={() =>
                                handleCopy(template.subject, "subject")
                              }
                          ...
                          <p className="text-sm font-medium p-2 bg-gray-50 border border-transparent rounded-sm">
                            {template.subject}
                          </p>
                        </div>
