...
export default function EarningsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response =
          (await dashboardService.getCommissions()) as unknown as CommissionsData;
        setData(response);
      } catch (error) {
    ...
    <div className="flex-1 w-full max-w-6xl mx-auto px-12 py-16 space-y-16">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-8 border-y border-gray-100 py-8">
        ...
        <div className="space-y-1 border-l border-gray-100 pl-8">
          ...
          <p className="text-3xl font-serif text-black">
            {stats.pendingEarnings.toLocaleString("fr-FR", {
        ...
        <div className="space-y-1 border-l border-gray-100 pl-8">
          ...
          <p className="text-3xl font-serif text-black">
            {stats.avgPerDeal.toLocaleString("fr-FR", {
              minimumFractionDigits: 2,
            })}{" "}
            $
          </p>
      ...
      <section className="space-y-8">
        ...
        <div className="divide-y divide-gray-50">
          {commissions
            .filter(
              (c) => selectedPeriod === "all" || c.status === selectedPeriod,
            )
            .map((commission) => (
              <div
                key={commission.id}
              ...
              >
                ...
                <div className="flex items-center gap-8">
                  <span
                    className={`text-[9px] uppercase tracking-widest ${getStatusColor(commission.status)}`}
                  >
