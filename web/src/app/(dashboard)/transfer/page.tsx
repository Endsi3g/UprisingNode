...
export default function TransferPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await transactionsService.createWithdrawal(parseFloat(amount));
      setSubmitted(true);
    } catch (error) {
      console.error("Transfer failed", error);
      // In a real app, I'd show a toast error here
    } finally {
          ...
          <div className="flex gap-3">
            {[1000, 2500, 5000].map((quickAmount) => (
              <button
                ...
                className={`flex-1 py-3 text-xs font-medium uppercase tracking-widest transition-all duration-200 border ${
