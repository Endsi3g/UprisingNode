...
export function ChatBubble({ message, isMe, timestamp, avatar }: ChatBubbleProps) {
    return (
        <div className={cn("flex gap-3 max-w-[80%]", isMe ? "ml-auto flex-row-reverse" : "")}>
...
export function ConversationItem({ name, lastMessage, time, unread, active, onClick }: ConversationItemProps) {
    return (
        <button
            onClick={onClick}
            className={cn(
        ...
        >
            ...
            <div className="flex-1 overflow-hidden">
                ...
                <p className={cn("text-xs truncate", unread ? "text-black font-medium" : "text-gray-500")}>
                    {lastMessage}
                </p>
            </div>
            {unread ? (
