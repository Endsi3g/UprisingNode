...
sed -i "s/async validate(payload: any) {/async validate(payload: { sub: string; email: string; role: string }) {\n    \/\/ eslint-disable-next-line @typescript-eslint/require-await/g" src/auth/jwt.strategy.ts
